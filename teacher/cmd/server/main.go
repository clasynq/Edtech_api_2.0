...
import (
	...
	"clasynq/api/teacher/config"
	delivery "clasynq/api/teacher/internal/delivery/http"
	"clasynq/api/teacher/internal/repository"
	"clasynq/api/teacher/internal/usecase"
...
func main() {
	...
	if cfg.Port == "" {
		cfg.Port = "8089" // default port for teacher service
	}
	...
	// 4. Initialize Layers
	repo := repository.NewPostgresTeacherRepository(db)
	uc := usecase.NewTeacherUsecase(repo, rdb)
	...
	r.GET("/ping", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"message": "pong from teacher service"})
	})

	handler.RegisterRoutes(r, authMiddleware)

	// 6. Start server
	log.Printf("Starting teacher service on port %s", cfg.Port)
